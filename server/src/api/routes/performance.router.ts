...
export function router(fastify: FastifyInstance, opts: FastifyOptions, next: FastifyDone): void {
  ...
  fastify.get('/setup/:id', getOneSetupPerformanceResponse, async (request: GetOneSetupPerformanceRequest, reply) => {
    const performance = PerformanceService.getSetupPerformanceById(request.params.id, request.query.gameId);
